# -*- coding: utf-8 -*-
"""NH-ROBO — o agente que roda na maquina do colaborador.

Feito a partir de uma COPIA do agente Q-Colabore 0.4.0 do Qualicontax,
entregue em 06/09/2026. Nada do Q-Colabore foi alterado: sao dois robos
irmaos, com pasta, chave, porta e servidor proprios, que convivem na mesma
maquina sem se atrapalhar. O que veio de la e o que custou um ano de
tropecos — o Defender marcando o --onefile, o arquivo ainda sendo copiado,
o caderninho que sobrevive a queda de energia, o recuo quando a rede cai.


Roda continuamente em C:\\nhrobo. Vigia as pastas que o funcionário escolheu,
envia cada arquivo novo para a nuvem (POST /api/nhrobo/enviar, autenticado pela
...
servidor (GET /api/nhrobo/config), nunca é decidida aqui.

Tecnologia: Python + Nuitka em modo PASTA (--standalone, ver build_nhrobo.ps1).
...
A CHAVE mora SÓ em C:\\nhrobo\\config.json — nunca em variável de ambiente,
...
ASSET_LOGO = "nhrobo_logo.png"        # logo do Grupo NH, no cabeçalho
ASSET_ICON = "nhrobo_icon.png"        # marca NH: ícone da janela e da bandeja
...
# ---------------------------------------------------------------------------
SERVIDOR_PADRAO = "https://app.postonovohorizonte.com.br"   # nuvem do Grupo NH
# Casa do agente: C:\nhrobo em produção. NHROBO_HOME só é usado pela prova
# automatizada, para não tocar na instalação real da máquina.
BASE_DIR = os.environ.get("NHROBO_HOME") or r"C:\nhrobo"
CONFIG_PATH = os.path.join(BASE_DIR, "config.json")
LOG_PATH = os.path.join(BASE_DIR, "nhrobo.log")
...
#   CALADO — e a pessoa juraria que instalou.
...
def autostart_ativo():
    """True se a entrada Run 'NHRobo' existe para este usuário."""
    if not winreg:
        return False
    try:
...
def testar_conexao(servidor, chave):
    """Bate no GET /api/nhrobo/config com a chave dada. Devolve:
    ...
    try:
        r = requests.get(servidor + "/api/nhrobo/config",
                        headers={"Authorization": "Bearer " + (chave or "").strip(),
                                 "User-Agent": "NHRoboAgente/%s" % __version__},
...
class Worker(threading.Thread):
    ...
    def __init__(self, estado):
        super().__init__(name="nhrobo-worker")
        self.estado = estado
    ...
    def _buscar_corte(self, cfg):
        """GET /api/nhrobo/config -> date de corte (ou None). Levanta
        ChaveInvalida em 401/403. Deixa outras exceções subirem (sem conexão)."""
        r = self.sessao.get(cfg["servidor"] + "/api/nhrobo/config",
    ...
    def _enviar(self, cfg, caminho):
        """POST /api/nhrobo/enviar. Devolve ('ok'|'rejeitado'|'retry', motivo).
        Levanta ChaveInvalida em 401/403 (para tudo lá em cima)."""
        nome = os.path.basename(caminho)
        with open(caminho, "rb") as f:
            r = self.sessao.post(
                cfg["servidor"] + "/api/nhrobo/enviar",
...
def rodar_gui():
    ...
    root = tk.Tk()
    root.title("NH-Robô %s — Configuração" % __version__)   # mesmo padrão do Q-Robô
    ...
    if p_logo:
        try:
            ...
            pass
    tk.Label(top, text="NH-Robô", font=("Segoe UI", 18, "bold"), bg="white"
    ...
    if tem_tray:
        ...
        tray_icon = pystray.Icon("nhrobo", _img_tray(), "NH-Robô", menu)
...
def main():
    ...
    if trava is None:
        ...
        try:
            ...
            import tkinter as tk
            r = tk.Tk(); r.withdraw()
            mb.showinfo("NH-Robô", "O agente já está em execução.")
            r.destroy()
        except Exception:
            pass
        return 0
