...
def importar_pedido(pedido_id):
    ...
    try:
        ...
        # Buscar itens do pedido com destino do cliente
        cursor.execute("""
            SELECT pi.*,
...
def salvar_importados():
    ...
    try:
        ...
        for i in range(len(clientes_ids)):
            ...
            cursor.execute("""
                INSERT INTO fretes (
                    ...
                    valor_total_frete, comissao_motorista, valor_cte, comissao_cte,
                    lucro, data_frete, status
                )
            ...
            """, (
                ...
                veiculo_id,
                qtd_id,
                converter_para_decimal(quantidades[i]) if not qtd_id else None,
                origens_ids[i],
                destinos_ids[i],
                converter_para_decimal(precos_unitarios[i]),
                converter_para_decimal(totais_nf[i]),
                converter_para_decimal(precos_por_litro[i]),
                converter_para_decimal(valores_totais_frete[i]),
                converter_para_decimal(comissoes_motorista[i]),
                converter_para_decimal(valores_cte[i]),
                converter_para_decimal(comissoes_cte[i]),
                converter_para_decimal(lucros[i]),
                data_frete,
                status_list[i]
            ))
            fretes_criados += 1
        ...
        flash(f'{fretes_criados} frete(s) criado(s) com sucesso! Pedido marcado como Faturado.', 'success')
...
def novo():
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO fretes (
                    ...
                    preco_produto_unitario, total_nf_compra, preco_por_litro,
                    valor_total_frete, comissao_motorista, valor_cte, comissao_cte,
                    lucro, data_frete, status, observacoes
                )
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
            ...
            # Se veio de um pedido, marcar como Faturado
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, razao_social, paga_comissao, percentual_cte, cte_integral, destino_id
        ...
        cursor.execute("""
            SELECT p.id, p.numero, p.data_pedido, p.status, p.motorista_id,
                   m.nome as motorista_nome
            FROM pedidos p
            LEFT JOIN motoristas m ON p.motorista_id = m.id
        ...
        if request.method == 'POST':
            ...
            cursor.execute("""
                UPDATE fretes SET
                    ...
                    total_nf_compra=%s, preco_por_litro=%s, valor_total_frete=%s, comissao_motorista=%s,
                    valor_cte=%s, comissao_cte=%s, lucro=%s, data_frete=%s, status=%s, observacoes=%s
                WHERE id=%s
            """, (
                request.form.get('clientes_id'),
                request.form.get('produto_id'),
